...
- `query_chunk_size` controls how many workload queries are attended in one cross-attention chunk. Set it at least as high as the pure workload query count when memory allows; smaller chunks are a scale-preserving approximation because each chunk has its own attention softmax.
