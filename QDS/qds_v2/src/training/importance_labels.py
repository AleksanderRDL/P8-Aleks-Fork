...
def _knn_representative_support(
    ...
    representatives_per_trajectory: int = 3,
) -> torch.Tensor:
    """Return nearest retained-worthy kNN candidates per selected trajectory."""
    ...
    anchor_lat = float(params["lat"])
    anchor_lon = float(params["lon"])
    anchor_time = float(params["t_center"])
    ...
    for trajectory_id in sorted(trajectory_ids):
        ...
        if candidate_offsets.numel() == 0:
            continue
        candidate_points = trajectory_points[candidate_offsets]
        spatial_distance = _haversine_km(candidate_points[:, 1], candidate_points[:, 2], anchor_lat, anchor_lon)
        temporal_distance = torch.abs(candidate_points[:, 0] - anchor_time)
        distance = spatial_distance + 0.001 * temporal_distance
        representative_count = min(max(1, representatives_per_trajectory), int(candidate_offsets.numel()))
        representative_offsets = candidate_offsets[torch.topk(-distance, representative_count).indices]
        support[start + representative_offsets] = True

    return support


def _similarity_support_mask(points: torch.Tensor, params: dict[str, float]) -> torch.Tensor:
    """Return points that could make their trajectory visible to a similarity query."""
    spatial_distance = torch.sqrt(
        (points[:, 1] - float(params["lat_query_centroid"])) ** 2
        + (points[:, 2] - float(params["lon_query_centroid"])) ** 2
    )
    return (
        (points[:, 0] >= float(params["t_start"]))
        & (points[:, 0] <= float(params["t_end"]))
        & (spatial_distance <= float(params["radius"]))
    )
...
def compute_typed_importance_labels(
    ...
    for q in typed_queries:
        ...
        if qtype == "range":
            ...
            gain = _set_query_singleton_gain(original_ids)
            if gain > 0.0:
                similarity_support = _similarity_support_mask(points, params)
                for trajectory_id in original_ids:
